...
#define i2c_lld_write_byte(i2cp) \
      i2cp->i2c->DR = *i2cp->txptr;  \
      i2cp->txptr++;  \
      i2cp->txbytes--
#define i2c_lld_read_byte(i2cp) \
      *i2cp->rxptr = (uint8_t)i2cp->i2c->DR; \
      i2cp->rxptr++; \
      i2cp->rxbytes--
#endif
...
#define I2C_EV8_MASTER_BYTE_TRANSMIT                                        \
  ((uint32_t)(((I2C_SR2_MSL | I2C_SR2_BUSY | I2C_SR2_TRA) << 16) |          \
              I2C_SR1_TXE))

#define I2C_EV8_1_MASTER_BYTE_TRANSMIT                                       \
  ((uint32_t)(((I2C_SR2_MSL | I2C_SR2_BUSY | I2C_SR2_TRA) << 16)))

#define I2C_EV7_MASTER_BYTE_RECEIVE                                         \
  ((uint32_t)(((I2C_SR2_MSL | I2C_SR2_BUSY) << 16) |                        \
              I2C_SR1_RXNE))

#define I2C_EV7_1_MASTER_LAST_BYTES_RECEIVED                                \
  ((uint32_t)(((I2C_SR2_MSL | I2C_SR2_BUSY) << 16) |                        \
...
/**
 * @brief   Stops an I2C transaction.
 *
 * @param[in] i2cp      pointer to the @p I2CDriver object
 *
 * @notapi
...
static void i2c_lld_serve_event_interrupt(I2CDriver *i2cp) {
  ...
  case I2C_EV8_MASTER_BYTE_TRANSMIT:
    ...
    } else { /* ref: Stop condition should be programmed during EV8_2 event, when either TxE or BTF is set */
      dp->CR2 &= ~I2C_CR2_ITBUFEN; /* stop receiving any more TxE interrupts */
      if (i2c_lld_get_rxbytes(i2cp) > 0U) {
        i2cp->addr |= 0x01;
        /* start will clear TxE and generate ReStart at end of current byte transfer */
...
static void i2c_lld_serve_error_interrupt(I2CDriver *i2cp, uint16_t sr) {
  ...
  if (i2cp->dmarx)
      dmaStreamDisable(i2cp->dmarx);
#else
  /* Clear interrupt flags. DMA-path will have already cleared I2C_CR2_ITEVTEN */
  i2cp->i2c->CR2 &= ~(I2C_CR2_ITEVTEN | I2C_CR2_ITBUFEN);
  i2cp->txbytes = 0;
  i2cp->rxbytes = 0;
#endif
